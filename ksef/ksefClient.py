...
class ksefClient:
    ...
    def _load_certificate(self) -> x509.Certificate:
        if self._certificate is None:
            if not os.path.exists(self.cert_path):
                raise ksefError.ksefError(f"Certificate not found: {self.cert_path}")
    ...
    def _load_private_key(self):
        if self._private_key is None:
            if not os.path.exists(self.key_path):
                raise ksefError.ksefError(f"Private key not found: {self.key_path}")
            ...
            try:
                ...
                raise ksefError.ksefError(f"Error loading private key: {e}")
    ...
    def _make_request(
        ...
        self.logger.info(f"KSeF Request: {method} {url}")
        if data:
            self.logger.debug(f"KSeF Request data: {json.dumps(data, indent=2)}")
        ...
        try:
            ...
            self.logger.info(f"KSeF Response: {response.status_code}")
            self.logger.debug(f"KSeF Response body: {response.text[:2000] if response.text else 'EMPTY'}")
            ...
            self.logger.error(f"KSeF Request Error: {e}")
            raise ksefError.ksefError(message=f"Connection error with KSeF: {str(e)}")
    ...
    def get_public_key(self) -> str:
        ...
        if not certificates:
            raise ksefError.ksefError("No public key certificates available from KSeF")
        ...
        if not cert_pem:
            raise ksefError.ksefError("Could not extract public key from KSeF response", response_data=response)
    ...
    def _encrypt_token_for_ksef(self, token: str, timestamp_ms: int) -> str:
        ...
        try:
            ...
            raise ksefError.ksefError(f"Failed to load public key: {e}")
    ...
    def init_session_token(self, nip: str) -> dict:
        ...
        if not self._ksef_token:
            raise ksefError.ksefError("No KSeF token configured. Use from_token() or provide token parameter.")

        self.logger.info(f"Getting challenge for NIP: {nip}")
        ...
        self.logger.info(f"Received challenge: {challenge[:20]}..., timestamp: {timestamp_ms}")

        self.logger.info("Encrypting token with KSeF public key...")
        encrypted_token = self._encrypt_token_for_ksef(self._ksef_token, timestamp_ms)

        self.logger.info("Sending encrypted token to /auth/ksef-token...")
        data = {
            "challenge": challenge,
            "contextIdentifier": {
                "type": "Nip",
        ...
        self.logger.info(f"Received authenticationToken, referenceNumber: {self.reference_number}")

        self.logger.info("Checking authorization status...")
        max_attempts = 30
        for attempt in range(max_attempts):
            ...
            if processing_code == 200:
                ...
                self.logger.info(f"Authorization in progress (attempt {attempt + 1}/{max_attempts})...")
                time.sleep(1)
            else:
            ...
            raise ksefError.ksefError(message="Authorization timeout")

        self.logger.info("Exchanging authenticationToken for accessToken...")
    ...
    def init_session_xades(self, nip: str) -> dict:
        ...
        self.logger.info(f"Received challenge: {challenge[:20]}...")

        self.logger.info("Building and signing AuthTokenRequest XML...")
        xml_content = self._build_auth_token_request_xml(challenge, nip, timestamp)
        signed_xml = self._sign_xml_xades(xml_content)

        self.logger.info("Sending signed XML to /auth/xades-signature...")
        response = self._make_request(
            'POST',
    ...
    def terminate_session(self) -> dict:
        if not self.access_token:
            raise ksefError.ksefError("No active session to terminate")
    ...
    def query_invoices(
        ...
        if (date_to - date_from) > max_range:
            self.logger.warning(f"Date range exceeds 90 days, limiting to {date_to - max_range} - {date_to}")
